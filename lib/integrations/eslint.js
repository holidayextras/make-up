...
EslintIntegration.RULESETURL = 'https://raw.githubusercontent.com/holidayextras/culture/ccc7731784ffa24223ec15381b703db56cdc49c4/.eslintrc';
...
EslintIntegration.run = function(options, output, callback) {
  ...
  if (fs.existsSync(EslintIntegration.ESLINTRC)) {
    return glob(globs, EslintIntegration._processGlobs.bind(EslintIntegration, options, output, callback));
  }

  EslintIntegration._downloadConfig(output, function(err) {
    if (err) return callback(err);
    glob(globs, EslintIntegration._processGlobs.bind(EslintIntegration, options, output, callback));
  });
};
